...
- **`load` param block appends 3 "Akaname" entries** (round 23 verification). asphyxia `load.pug` L155-160 appends `each id in [0,1,2]: info{ type=6, id=#{id}, param(__count=1)=#{akaname} }` AFTER the normal params. `LoadHandler` builds these with `ParamInfo{Type=6, Id=id, Param=[profile.Akaname]}`. Verified against live sv7_load: 12 normal `<info>` entries followed by 3 `type=6 id=0/1/2 __count=1` entries — matches asphyxia byte-for-byte. No change needed; documented to prevent regression.
- **`viiMigrate` must reset `packets`/`blocks` to 0, not 10000** (round 23). asphyxia `migrate.ts` `viiMigrate` L190-225 `$set`s `blocks: 0, packets: 0` on the migrated v7 profile (counters/currency are all reset; only `name`, `effCLeft`, `effCRight`, `hiSpeed`, `laneSpeed`, `bplSupport`, `creatorItem` are carried over). Stella previously set `Packets = 10000, Blocks = 10000` as an "enhancement" — a divergence. `MigrationHelper.ViiMigrateAsync` now sets `Packets = 0, Blocks = 0` to match asphyxia. Also the migrated `SvScore.PlayCount` is now `0` (asphyxia's v6/v7 `saveScore`/`viiMigrate` never set `playCount` — the field stays default 0; `loadScore` for v6/v7 does not emit `cnt`/`playCount` in the param array, so this is wire-invisible but kept at 0 for schema parity).
- **`lounge` `wait` must be 0, not the room's `sec`** (round 23). asphyxia `features.ts` `lounge` L309-320 computes `longestWait = Math.max(...matches.map(m => m.sec))`, but `globalMatch` never stores `sec` on the room object (the pushed room has `version/c_ver/filter/mid/p_rest/p_num/players` only — `sec` is captured only in the `setTimeout` closure). So `m.sec` is `undefined`, `Math.max` yields `NaN`, and `K.ITEM('u32', NaN)` serialises as `0`. `LoungeInternal` now returns `Wait = 0` when rooms exist (and `Interval = 5` with no `wait` when none exist), matching asphyxia's effective behaviour. The `MatchRoom.Sec` field is retained for the deletion timer but is not read by `lounge`.

- **`arena` open logic must use `arena_no_endtime`, not a separate `arena_open` flag** (round 23). asphyxia `common.ts` L496 + `profiles.ts` L947 compute `arenaOpen = U.GetConfig('arena_no_endtime') || BigInt(date) < currentArena.time_end` — asphyxia registers NO `arena_open` config (only `arena_no_endtime`, `arena_station`). Stella previously used `cfg.ArenaOpen` (a Stella-invented config) for the arena-open check, so the WebUI `arena_no_endtime` toggle did nothing. `CommonHandler.BuildArena` and `LoadHandler.LoadInternal` now use `cfg.ArenaNoEndtime` (with `cfg.ArenaOpen` OR'd in as a retained Stella-specific force-open so the extra WebUI toggle still functions; when `arena_open=false` the logic is byte-identical to asphyxia).
- **`arena` `is_shop`/catalog must use `arena_station !== 'None'`, not `arena_session`** (round 23). asphyxia `common.ts` L497: `shopOpen = arenaOpen && U.GetConfig('arena_station') !== 'None'`; the catalog is rendered only when `shopOpen && arena_station !== 'None' && version >= arenaItems[arena_station].version`. Stella previously used `cfg.ArenaSession != 0` for `shopOpen` (a Stella-invented config that asphyxia does not have). `BuildArena` now uses `(cfg.ArenaStation ?? "None") != "None"` for `shopOpen` and the catalog gate, matching asphyxia. `arena_session` is retained in the WebUI/config but is no longer read by the arena logic.
- **`load` arena lookup must filter by the CURRENT season** (round 23). asphyxia `profiles.ts` L952 looks up the arena record by `{season: arenaOpen ? currentArena.season : 0, version}` — i.e. only the current (or 0) season's row, not any row for the profile. Stella previously did `SingleOrDefaultAsync(a => a.Profile == .. && a.Version == ..)` with NO season filter, which (a) diverged from asphyxia and (b) would throw `InvalidOperationException` if a profile had arena rows for multiple seasons (SingleOrDefault requires ≤1). `LoadInternal` now resolves `currentArena` for the game version, computes `arenaOpen`, and filters `a.Season == (arenaOpen ? currentArena.Season : 0)`. With no matching row, `arena` stays null and `LoadArenaElement?` is omitted (asphyxia pug `if arena`).
- **`common` `arena` element must be EMPTY (`<arena/>`) when arena is closed** (round 23). asphyxia initialises `arenaData = {}` and only populates it when `arenaOpen && version >= 20220425 && currentArena.season !== 0`; otherwise `arena: {}` serialises as an empty `<arena></arena>` with NO children. Stella's `ArenaElement` previously declared non-nullable value-type fields (`int Season`, `ulong TimeStart`, `bool IsOpen`, …) so a closed arena serialised as `<arena><season>0</season>…<is_shop>false</is_shop></arena>` — extra zero-valued children the game does not expect. All `ArenaElement` leaf fields are now nullable (`int?`, `ulong?`, `bool?`) with `[XmlElement]` (safe — nullable VALUE types are only rejected with `[XmlAttribute]`, not `[XmlElement]`); `BuildArena` returns `new ArenaElement()` when closed, so XmlSerializer omits every null child and the wire output is `<arena/>`, byte-for-byte like asphyxia. Verified: sv7 (currentArena.season=0) → `<arena/>`; sv6 (season=22, arena_no_endtime=true, arena_station='None') → full arena with `is_open=1`, `is_shop=0`, no catalog.
